...
export default function HomeScreen() {
  ...
  const onMenuPress = () => router.push({ pathname: "/legacy", params: { to: item.label } });

  const onShortcutPress = (item: ShortcutItem) => {
    // Phase 1 routing: everything routes to legacy for now
    // (Legacy will open on its Home tab — expected.)
    router.push({ pathname: "/legacy", params: { to: item.label } });
    // If you ever want “Coming soon” behaviour again, we can re-add it here.
  };
