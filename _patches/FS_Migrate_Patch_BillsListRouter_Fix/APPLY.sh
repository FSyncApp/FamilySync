...
# Patch folder: <repo>/_patches/FS_Migrate_Patch_BillsListRouter_Fix/APPLY.sh
# Repo root is two levels up from this script's directory.
ROOT="$(cd "$(dirname "$0")/../.." && pwd)"

echo "== FamilySyncMigrate patch FIX: Router-native Bills list (apply into repo root) =="
echo "Repo root: $ROOT"
cd "$ROOT"
...
# Clean up any accidental files written into _patches from the prior failed attempt.
if [ -d "_patches/app" ]; then
  echo "Removing accidental _patches/app folder from previous failed apply"
  rm -rf "_patches/app"
fi

# Remove the legacy boundary file if present (we are moving Bills to Router-native folder route).
...
if [ -f "app/(tabs)/index.tsx" ]; then
  ...
  echo "ERROR: Expected Home at app/(tabs)/index.tsx but not found."
  exit 1
fi
...
echo "Patching Home shortcut handler so Bills routes to /bills"
python3 - <<'PY'
import re
...
echo "Done."
echo "Next: reload (press r) and tap Bills. You should see Router-native Bills (no legacy banner)."
